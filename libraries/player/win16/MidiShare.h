...
#else
#  define MIDISHAREAPI WINAPI _export
#	define MSALARMAPI	WINAPI _export
#endif

#ifdef __UseMSThunking__
#	ifdef __UseMSOldInterface__
#	undef __UseMSOldInterface__
#	endif
#endif
...
/*------------------------------ sequence header ---------------------------*/
	...
	typedef MidiSeqPtr MidiSeqPtr;
	

/*-------------------------------- input Filter -------------------------------*/

	typedef struct TFilter FAR *FilterPtr, FAR * MidiFilterPtr;
	...
	typedef FilterPtr MidiFilterPtr;

#else
	typedef long MidiEvPtr;
	typedef long MidiSeqPtr;
	typedef long MidiFilterPtr;
#endif

#ifdef __Macintosh__
	typedef unsigned char FAR *MidiName;
#else
...
typedef void (MSALARMAPI *TaskPtr)		( unsigned long date, short refNum, long a1,long a2,long a3 );
...
/*------------------------------- Sequence managing ---------------------------*/
  ...
  MidiEvPtr	MIDISHAREAPI MidiGetFirstEv	(MidiSeqPtr seq);
  void		MIDISHAREAPI MidiSetFirstEv	(MidiSeqPtr seq, MidiEvPtr e);
  MidiEvPtr	MIDISHAREAPI MidiGetLastEv	(MidiSeqPtr seq);
  void		MIDISHAREAPI MidiSetLastEv	(MidiSeqPtr seq, MidiEvPtr e);
