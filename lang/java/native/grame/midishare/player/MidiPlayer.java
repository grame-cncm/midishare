...
/*****************************************************************************/
/* MidiPlayer Class : interface to the native Player library
/*
/* 26/07/97  New InsertTrack et InsertAllTrack functions
/* 25/11/97  New kExternalSync mode and SetTempo function
/* 30/10/98  Remove the kEventSync mode, kExternalSync get the value 3
/*****************************************************************************/
...
/**
This class in an interface to the native Player library. It allows to 
develop multi-tracks, synchronisable MidiShare players.
Each Player is a MidiShare application, has 256 tracks, four different 
synchronization mode, events chase and loop markers. 
...
public final class MidiPlayer {
	...
	/**
	Open a new empty Player.
	The tempo is assumed to be 120 bpm, the time signature is 4/4 and the internal 
	resolution is set by default to 500 ticks_per_quarter so that the internal time 
	unit (tick) correspond exactly to one millisecond. Any opened Player should be closed using 
	the Close function.
	
	*@param name is the name of the Player (actually the name of the associated
	...
	*/
	...
	recorded events  are then inserted in the track. By default the recording state is 
	set in kMixMode.
	*@param refnum is the Player reference number. 
	...
	*/
	...
	RecordPlayer called with kNoTrack parameter will disable all recording. The 
	track being recorded can be changed while recording.  

	*@param refnum is the Player reference number. 
	*@param tracknum the number of the track to be set in record mode: from 0 to 255.  Parameter 
	...
	*@param refnum is the Player reference number. 
	*@param filter is a pointer to a Midi filter.
	...
	*/
	...
	position in musical time (Bar, Beat, Unit). This function can be used when the 
	Player is moving but only is kInternalSync mode.
	*@param refnum is the Player reference number. 
	...
	*/
	...
	millisecond. This function can be used when the Player is moving but only is 
	kInternalSync mode.
	*@param refnum is the Player reference number. 
	...
	*/
	...
	(Bar, Beat, Unit). This function can be used when the Player is moving but only 
	in kInternalSync mode.
	*@param refnum is the Player reference number. 
	*@param pos  is a PlayerPos object.
	*/
	...
	*@param refnum is the Player reference number. 
	*@param pos  is a PlayerPos object.
	*@return The result is an error code. This error is returned when trying to set a Loop 
	end marker before a the Loop start marker or a Loop start after a Loop end 
	marker.
	*/
	...
	function can be used when the Player is moving but only is kInternalSync mode.
	*@param refnum is the Player reference number. 
	*@param date  is a date in millisecond.
	*@return The result is an error code. This error is returned when trying to set a Loop 
	end marker before a the Loop start marker or a Loop start after a Loop end 
	...
	*/
	...
	<br>
	- kClockSync : The Player recognizes MIDI Clock at it's input and can be 
	driven by  Start, Stop, Continue, Clock and SongPos Midi messages.
	<br>
	- kSMPTESync : This is a global Synchronization mode for all MidiShare applications. 
	<br>
	- kExternalSync : The Player tempo can be changed using the SetTempo function or when external
	Tempo events are received.
	<br>	
	...
	*/
	...
	kClockSyncOut. In this case start, stop, continue, and clock Midi messages 
	are sent when the Player is running. By default, the synchronization mode is 
	kNoSyncOut.
	*@param refnum is the Player reference number. 
	...
	*/
	...
	(tempo in micro-second/per/quarter-note = 60 000 000 / tempo in beat/per/min)
	*@param refnum is the Player reference number. 
	*@param tempo is the new tempo value to be set.
	*/
	...
	*/
	...
	/**
	Returns the state of the Player at the current position. This function is usually 
	used for state displaying purpose. 
	*@param refnum is the Player reference number. 
	...
	*@param refnum is the Player reference number. 
	*@param state a PlayerState object, will be filled with information about the last event in the score : position in 
	...
	*/
	...
	/**
	This function allows to implement step playing. ForwardStep plays 
	the next group of notes (notes at the same date)  and allows to move in the 
	score by group of notes. ForwardStepPlayer takes care of the tracks state (mute or solo mode).
	...
	*/
	...
	the previous group of notes (notes at the same date) and allows to move in the 
	score by group of notes. BackwardStepPlayer takes care of the tracks state (mute or solo mode).
	...
	*/
	...
	will be erased before the new track is set.
	The MidiShare sequence given as parameter will be internally used. It means 
	that one must copy it before using  SetTrack if one wants to keep it.
	*@param refnum is the Player reference number. 
	*@param tracknum is the  track number.
	*@param seq is a pointer on a sequence to be set in the Player.
	*@return The result is an error code. This error is returned if the SetTrack function is used
	...
	*/
	...
	The MidiShare sequence given as parameter will be internally used. It means 
	that one must copy it before using  SetAllTrack if one wants to keep it.
	*@param refnum is the Player reference number. 
	*@param seq is a pointer on a sequence to be set in the Player.
	*@param tick_per_quarter  : the number of ticks per quarter note, usually read in the MIDIfile header.
	*@return The result is an error code. This error is returned if the SetAllTrack function is used
	when the Player is running or if the sequence could not be inserted.
	*/
	...
	played solo.
	*@param refnum is the Player reference number. 
	*@param tracknum is the track number to be set.
	...
	*@param param is the parameter to be read (kMute or kSolo).
  	...
  	static {
		if (!interfaceLoaded){
			try {
				...
				System.err.println("JPlayer.library not found");
		  }
		}
	 }
 }