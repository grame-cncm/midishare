...
;; Check the Listener window for the result.It must be T. If it is not, you must 
...
;; Other received events are deleted. The program loops until the mouse is clicked.
...
(defun transform (transpose delay)
  ...
  (loop (if (mouse-down-p) (return))		; loop until mouse clicked
        (do ((event (MidiGetEv *refnum*) (MidiGetEv *refnum*)))	;read all the events
            ((%null-ptr-p event))		; in the rcv fifo
          (if (member (type event) (list typeNote typeKeyOn typeKeyOff))
            (progn					; note, keyOn and KeyOff events
...
(defparameter *midishare-process* 
  (process-run-function '(:name "midi-thru" :priority 2) #'midi-thru *refnum*))	; <== EVALUATE THIS EXPRESSION.
...
(progn
  ...
  (defpascal eval-task (:long date :word refnum :long index :long arg2 :long arg3)
...
(defun echo (delay attenuation)
  #'(lambda (refnum)
      (do ((event (MidiGetEv refnum) (MidiGetEv refnum)))
        ...
        (when (or (and (= (type event) typeKeyOn) (> (vel event) 0))
                  (= (type event) typeNote))
          (do ((dt (+ (date event) delay) (+ dt delay))
               (vl (- (vel event) attenuation) (- vl attenuation))
               (note ))
              ((or (< vl 1)(> vl 127)))
            (unless (%null-ptr-p (setq note (MidiNewEv typeNote)))
