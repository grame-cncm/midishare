...
{
	GrafPtr curPort; Rect r;
	
	SetRect(&scrollRect,11, 24, 274, 278);
	SetRect(&r, 0, 0, 274, 278);
	scrollRgn = NewRgn ();
	if (!scrollRgn) return;

	if (NewGWorld (&myPort, 0, &r, 0, 0, 0) != noErr) return;
	GetPort (&curPort);
	SetPort (myPort);
...
void DrawMyContent() 
{
	CopyBits (GetPortBitMapForCopyBits(myPort), GetPortBitMapForCopyBits(GetWindowPort(myWindow)), &scrollRect, &scrollRect, srcCopy, 0);
}
