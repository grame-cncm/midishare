...
{
	Rect r, portRect; short top;

	GetWindowPortBounds(myWindow, &portRect);
	top = topPortArea(portRect);
	SetRect (&r, kPortLeft, top, kPortLeft + kPortArea, top + kPortArea);
	InvalWindowRect (myWindow, &r);
}
...
/* -----------------------------------------------------------------------------*/
static short StringLength (char *ptr)
{
	short n = 0;
	while (*ptr++) n++;
	return n;
...
{
	TDriverInfos drvInfo; TSlotInfos slotInfo; SlotRefNum sref;
	Cell srcCell, dstCell; Boolean visible; RgnHandle rgn;
			...
			for (j = 1; j <= drvInfo.slots; j++) {
				...
				if (MidiGetSlotInfos (sref, &slotInfo)) {
					...
					if (slotInfo.direction & MidiInputSlot) {
						...
						LSetCell(slotInfo.name, StringLength(slotInfo.name), srcCell, theList[SrcL]);
						srcCell.v++; visible = true;
					}
					if (slotInfo.direction & MidiOutputSlot) {
						LAddRow(1, dstCell.v, theList[DstL]);
						LSetCell(slotInfo.name, StringLength(slotInfo.name), dstCell, theList[DstL]);
	...
	rgn = NewRgn();
	if (rgn) {
		GetWindowRegion (myWindow, kWindowContentRgn, rgn);
		LUpdate(rgn, theList[SrcL]);
		LUpdate(rgn, theList[DstL]);
		DisposeRgn (rgn);
	}
}
...
{
	Rect r;
	short l, c, w = kPortWidth + kPortSep, top;
