...
void MMError (char *s, SlotRefNum ref, int errCode, short in)
{
	...
	if (strcmp(buff, "yes") == 0) {
		if (errFile) {
			HANDLE h = CreateFile(errFile, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			if (h != INVALID_HANDLE_VALUE) {
				DWORD written;
				SetFilePointer(h, 0, 0, FILE_END);
				WriteFile(h, string, (DWORD)strlen(string), &written, NULL);
				CloseHandle(h);
			}
		}
...
void MMTrace (const char *s, long value)
{
	...
	if (errFile) {
		...
		if (h != INVALID_HANDLE_VALUE) {
			...
			sprintf_s (string, "%s : %ld\n", s, value);
			SetFilePointer(h, 0, 0, FILE_END);
			size_t len = strlen(string);
			WriteFile(h, string, (DWORD)len, &written, NULL);
			CloseHandle (h);
		}
	}
}
