 ...
 * Test des différentes fonctions de MidiShare.
 *
 * GRAME	Lundi 7 Décembre 1992	DF
 */
...
static Boolean Environment()
{
	...
	if( refNum= MidiOpen( ApplName))
	{
		...
		if( n!= count)
		{
			...
			fprintf( stdout, "Warning : nbre d'applications présentes incohérent !\n", n);
		}
...
static void OpenClose()
{
	...
	n= MidiCountAppls();
	fprintf( stdout, "    nombre renvoyé par MidiShare : %d\n", n);
	for( ; i>=0; i--)
		MidiClose( r[i]);
	n= MidiCountAppls();
	if( n!= 2)
		fprintf( stdout, "Warning : toutes les appls ne sont refermées : reste %d\n", n);
}
...
static void ApplConfiguration()
{
	...
	if( s= MidiGetName( refNum))
	{
		...
		if( strcmp( s, ApplName))
			fprintf( stdout, "Warning : nom incohérent !\n");
#endif
#ifdef PASCALNAME
		s[s[0]+1]=0;
		fprintf( stdout, "%s %s\n", &s[1], OK);
	...
	fprintf( stdout, "%lx %s\n", info, OK);
	if( info)
		fprintf( stdout, "Warning : zone info non positionnée et non nulle\n");
	...
	fprintf( stdout, "%lx %s\n", f, OK);
	if( f)
		fprintf( stdout, "Warning : filtre non positionné et non nul\n");
	...
	fprintf( stdout, "%lx %s\n", rcv, OK);
	if( rcv)
		fprintf( stdout, "Warning : alarme non positionnée et non nulle\n");
...
static void Connections( Boolean isFreeMem)
{
	...
	if( MidiIsConnected( 0, refNum) || MidiIsConnected( refNum, 0))
		fprintf( stdout, "Warning : application déjà connectée !");
	...
	if( !in || !out)
		if( isFreeMem)
			fprintf( stdout, "Warning : connections non établies : in %d, out %d\n",
					(int)in, (int)out);
...
static void Events( Boolean isFreeMem)
{
	...
	if( isFreeMem)
	{
		...
		fprintf( stdout, "alloués : %ld %s\n", l, OK);
		total+= l;
		l= MidiTotalSpace();
		if( (l!= total) && MidiCountAppls())
			fprintf( stdout, "Warning : TotalSpace incohérent : %ld FreeSpace : %ld\n"
						, l, MidiFreeSpace()); 
		total= l;
	}
	...
	fprintf( stdout, "%lx %s\n", e, OK);
	if( e && !isFreeMem)
		fprintf( stdout, "Warning : résultat incohérent : %lx\n", e); 
	if( (l= MidiFreeSpace())!= free-1)
		if( isFreeMem)
			fprintf( stdout, "Warning : FreeSpace incohérent : %ld\n", l); 
	...
	else if( isFreeMem)
	{
		...
		fprintf( stdout, "    MidiCopyEv : non testé\n");
		fprintf( stdout, "    MidiFreeEv : non testé\n");
	}
	...
	if( e)
	{
		...
		if( (l+1)!= MidiGetField( e, 0L))
			fprintf( stdout, "Warning : champs mal positionné !\n");
	}
	else
	{
		fprintf( stdout, "    MidiSetField : non testé\n");
		fprintf( stdout, "    MidiGetField : non testé\n");
		fprintf( stdout, "    MidiAddField : non testé\n");
		fprintf( stdout, "    MidiCountFields : non testé\n");
	}
}
...
static void Sequences( Boolean isFreeMem)
{
	...
	fprintf( stdout, "%lx %s\n", seq, OK);
	if( seq && !isFreeMem)
		fprintf( stdout, "Warning : séquence non nulle !\n");
	
	if( !isFreeMem) return;
	if( !seq)
	{
		fprintf( stdout, "Warning : séquence nulle !\n");
		fprintf( stdout, "    MidiAddSeq : non testé\n");
		fprintf( stdout, "    MidiFreeSeq : non testé\n");
		fprintf( stdout, "    MidiClearSeq : non testé\n");
		fprintf( stdout, "    MidiApplySeq : non testé\n");
		return;
	}
	...
	if( e= MidiNewEv( typeNote))
	{
		...
		if( (n=CountOfEvents( seq))!= 1)
			fprintf( stdout, "Warning : nbre d'evts incohérent : %ld\n", n);
		
		fprintf( stdout, "    MidiApplySeq : ");flush;
		MidiApplySeq( seq, ApplyProc);
		fprintf( stdout, "%s\n", OK);
		if( Pitch(e)!= 61)
			fprintf( stdout, "Warning : apply non effectué !\n");
...
static void Sending()
{
	...
	if( (r= MidiOpen( NewName)) < 0)
	{
		...
		fprintf( stdout, "          tests non effectués.\n");
		return;
	}
//	MidiConnect( refNum, 0, true);
	...
	if( e= MidiNewEv( typeKeyOn))
	{
		...
		if( copy= MidiCopyEv( e))
		{
			...
			fprintf( stdout, "%ld %s\n", n, OK);
			if( n!= 1)
				fprintf( stdout, "Warning : nbre d'evts incohérent !\n");
		...
		if( copy= MidiCopyEv( e))
		{
			...
			n= MidiCountEvs(r);
			if( n!= 1)
				fprintf( stdout, "Warning : nbre d'evts transmis incohérent : %ld\n", n);
...
static void Mail()
{
	...
	b= MidiWriteSync( &a, (void* )2);
	if( a!= 2)
		fprintf( stdout, "Warning : variable non nulle mais positionnée : %ld\n", a);
	if( b!= (void*)1)
...
static void Tasks( Boolean isFreeMem)
{
	...
	fprintf( stdout, "%s\n", OK);
	if( (p1!= 1) && isFreeMem)
		fprintf( stdout, "Warning : tache non effectuée !\n");
	...
	fprintf( stdout, "%ld %s\n", p1, OK);
	if( (p1!= 1) && isFreeMem)
		fprintf( stdout, "Warning : tache non distribuée !\n");
...
static void NoMem()
{
	...
	fprintf( stdout, "\n\nTolérance au défaut de mémoire :\n");
	free= MidiFreeSpace();
	seq= GetAllFreeEvs();
	l = MidiFreeSpace();
	...
	if( free!= (l=MidiFreeSpace()))
		fprintf( stdout, "Warning : freeSpace incohérent : %ld au lieu de %ld\n", l, free);
}
